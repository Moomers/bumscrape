...
def get_listings(active_only = True):
    query = [
        "SELECT url, l.spider, c.last_crawled, date_format(MAX(visited), '%c/%e') AS last_seen, date_format(MIN(visited), '%c/%e') AS first_seen, COUNT(*) AS visits, title, concat('$', format(c.price, 2)) AS price, num_tickets, posted",
